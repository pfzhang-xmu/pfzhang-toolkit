...
/**
 * DSH mode: the canvas runs inside the DSH Web GUI; product analytics are
 * disabled entirely (no Google tags, no analytics tool calls).
 */
const IS_DSH_EMBEDDED =
...
export function trackCanvasOpened() {
  if (IS_DSH_EMBEDDED) return Promise.resolve()
...
export async function sendTrackedWidgetMessage(
  ...
  return await sendMessage(message, analyticsContext)
}
